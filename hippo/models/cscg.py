...
class CHMM(object):
    def __init__(
        self, n_clones, x, a, pseudocount=0.0, dtype=np.float32, seed=42
    ):
    ...
    def sample_sym(self, sym, length):
        ...
        for _ in range(length):
            ...
            long_alpha = np.dot(
                alpha,
                T_weighted[state_loc[obs_tm1] : state_loc[obs_tm1 + 1], :],
            )
            long_alpha /= long_alpha.sum()
            idx = np.random.choice(
                np.arange(self.n_clones.sum()), p=long_alpha
            )
...
def updateC(C, T, n_clones, mess_fwd, mess_bwd, x, a):
    state_loc = np.hstack(
        (np.array([0], dtype=n_clones.dtype), n_clones)
    ).cumsum()
    mess_loc = np.hstack(
...
def forward(T_tr, Pi, n_clones, x, a, store_messages=False):
    ...
    for t in range(1, x.shape[0]):
        ...
        message = np.ascontiguousarray(
            T_tr[aij, j_start:j_stop, i_start:i_stop]
        ).dot(message)
        p_obs = message.sum()
        assert p_obs > 0
        message /= p_obs
...
def forward_mp(T_tr, Pi, n_clones, x, a, store_messages=False):
    ...
    for t in range(1, x.shape[0]):
        ...
        for d in range(len(new_message)):
            new_message[d] = (
                T_tr[aij, j_start + d, i_start:i_stop] * message
            ).max()
        message = new_message
        p_obs = message.max()
        assert p_obs > 0
...
def backtrace(T, n_clones, x, a, mess_fwd):
    ...
    for t in range(x.shape[0] - 2, -1, -1):
        ...
        t_start, t_stop = mess_loc[t : t + 2]
        belief = (
            mess_fwd[t_start:t_stop]
            * T[aij, i_start:i_stop, j_start + code[t + 1]]
        )
        code[t] = rargmax(belief)
